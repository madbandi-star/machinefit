...
function seriesFlags(entry) {
  const blob = [
    ...
    .toLowerCase();
  return {
    iso: /iso[- ]?lateral|아이소/.test(blob),
    plate: /plate[- ]?loaded|플레이트/.test(blob),
    select: /\bselect\b|셀렉터/.test(blob),
    ...
    linear: /linear|수평|리니어/.test(blob),
...
function introKo(entry, family, flags) {
  if (entry.verification_status === 'BRAND_MODEL_NOT_FOUND') {
    return '해머 스트렝스 전용 모델이 없는 카테고리입니다. 헬스장에 있는 실제 기구의 패드·레버·안전장치를 먼저 확인한 뒤, 아래 패턴으로 움직이세요.';
  }
  const bits = [];
  ...
  if (flags.select) bits.push('Select 스택');
  const flavor = bits.length ? `${bits.join(' · ')} 구조입니다.` : '해머 스트렝스 궤적을 그대로 타는 게 핵심입니다.';
...
function introEn(entry, family, flags) {
  if (entry.verification_status === 'BRAND_MODEL_NOT_FOUND') {
    return 'There is no dedicated Hammer Strength SKU for this category. Confirm pads, levers, and safeties on the unit in your gym, then follow the movement pattern below.';
  }
  const bits = [];
  ...
  if (flags.select) bits.push('Select stack');
  const flavor = bits.length
    ? `Lean into the ${bits.join(' / ')} design.`
    : 'Ride the Hammer Strength path instead of fighting it.';
...
function buildTipKo(entry) {
  ...
  const parts = [
    `🏋️ HAMMER STRENGTH — ${model}${series}`,
    '',
    '🎯 ONE KEY CUE',
    `🔥 "${cue}"`,
    '',
