-- Import TGS_STRENGTH MachineFit PRO tips (trainer coaching style).
-- Source: database/catalog/pro-tips/tgs_strength_pro_tips.csv
-- Backup previous pro_tips / pro_tips_meta before UPDATE.

CREATE TABLE IF NOT EXISTS _backup_tgs_strength_pro_tips_20260820 (
...
INSERT INTO _backup_tgs_strength_pro_tips_20260820 (machine_id, code, machine_name_ko, pro_tips, pro_tips_meta)
SELECT m.id,
       m.code,
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 체스트 프레스

🎯 ONE KEY CUE
🔥 "견갑을 붙인 채 손잡이를 가슴 중앙으로 밀기"

TGS 스트렝스 전용 모델이 없는 카테고리입니다. 헬스장에 있는 실제 기구의 패드·레버·안전장치를 먼저 확인한 뒤, 아래 패턴으로 움직이세요.
...
"견갑을 붙인 채 손잡이를 가슴 중앙으로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Chest Press

🎯 ONE KEY CUE
🔥 "Brace scapulae and press the handles through center chest"

There is no dedicated TGS Strength SKU for this category. Confirm pads, levers, and safeties on the unit in your gym, then follow the movement pattern below.
...
"Brace scapulae and press the handles through center chest. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 등판, 시작용 로드 추출 풋 지지, 중량","importedAt":"2026-08-20T05:10:23.421Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
  AND m.is_active = TRUE
  AND COALESCE(
    (
      SELECT st.name->>'ko'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 인클라인 체스트 프레스
...
"인클라인 궤적에서 쇄골·상부 가슴 방향으로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Incline Chest Press
...
"Press along the incline path toward upper chest. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Incline Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 등판 각도, 시작용 로드 추출 풋 지지, 중량","importedAt":"2026-08-20T05:10:23.421Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 디클라인 체스트 프레스
...
"하부 가슴을 향해 아래·앞으로 통제하며 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Decline Chest Press
...
"Press down and forward toward the lower chest with control. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Decline Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/등판, 손잡이 시작, 중량","importedAt":"2026-08-20T05:10:23.421Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 컨버징 체스트 프레스
...
"넓은 시작에서 중앙으로 모으며 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Converging Chest Press
...
"Start wide and press while converging toward center. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Converging Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 암 시작 폭, 중량","importedAt":"2026-08-20T05:10:23.421Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 체스트 프레스
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Chest Press
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 좌·우 독립 암, 시작용 풋 지지, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 플레이트로드 체스트 프레스
...
"견갑을 고정한 채 양팔을 고르게 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Plate-Loaded Chest Press
...
"Keep scapulae set and press both arms evenly. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Plate-Loaded Chest Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/등판, 양쪽 플레이트 로딩, 시작 위치","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 펙덱
...
"팔꿈치를 패드에 붙인 채 가슴 앞에서 모으기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Pec Deck
...
"Keep elbows on the pads and bring the arms together in front of the chest. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Pec Deck 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔 패드 높이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 리어 델트 / 리버스 펙덱
...
"가슴을 패드에 붙이고 견갑을 모으며 팔을 뒤로 벌리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Rear Delt / Reverse Pec Deck
...
"Keep the chest on the pad and open the arms back while retracting scapulae. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Rear Delt / Reverse Pec Deck 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 손잡이/암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 플라이 머신
...
"팔꿈치를 약간 굽힌 채 가슴 앞에서 호를 그리며 모으기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Fly Machine
...
"With soft elbows, arc the arms together in front of the chest. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Fly Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 암 시작 위치, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 딥 머신
...
"팔꿈치를 몸통 가까이 두고 손잡이를 아래로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Dip Machine
...
"Keep elbows close to the torso and press the handles down. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Dip Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 손잡이 폭, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 어시스트 딥
...
"보조 무게로 깊이를 통제하며 딥 핸들을 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Assisted Dip
...
"Use assist load to control depth while pressing the dip handles. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Assisted Dip 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"무릎/보조 패드, 손잡이, 보조 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 슈퍼 인클라인 프레스
...
"가파른 인클라인에서 상부 가슴·전면 삼각으로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Super Incline Press
...
"Press on a steep incline toward upper chest and front delts. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Super Incline Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 고각도 등판, 시작용 풋 지지, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 랫풀다운
...
"가슴을 열고 팔꿈치를 옆구리로 내리며 바를 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Lat Pulldown
...
"Open the chest and drive elbows to the ribs while pulling the bar down. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Lat Pulldown 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"허벅지 고정 패드, 시트, 그립, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 와이드 랫풀다운
...
"와이드 그립으로 팔꿈치를 아래로 내리며 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Wide Lat Pulldown
...
"With a wide grip, drive the elbows down while pulling. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Wide Lat Pulldown 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"허벅지 고정 패드, 와이드 바, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 프론트 풀다운
...
"바를 쇄골 앞쪽으로 당기며 광배를 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Front Pulldown
...
"Pull the bar to the front of the collarbone and squeeze the lats. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Front Pulldown 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"허벅지 고정 패드, 시트, 바, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 랫풀다운
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Lat Pulldown
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Lat Pulldown 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 허벅지 패드, 좌·우 독립 암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 하이로우
...
"높은 시작에서 팔꿈치를 옆구리로 당기며 광배·후면 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — High Row
...
"From a high start, pull elbows to the ribs and squeeze mid-back. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 High Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 하이로우
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral High Row
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral High Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 좌·우 독립 암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 시티드 로우
...
"가슴을 패드에 붙이고 팔꿈치로 당기며 견갑을 모으기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Seated Row
...
"Keep the chest on the pad and row with the elbows while retracting scapulae. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Seated Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 풋 지지, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 로우 머신
...
"중립 자세에서 팔꿈치로 당기며 중부 등을 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Row Machine
...
"From a neutral setup, row with the elbows and squeeze mid-back. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Row Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 풋 지지, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 로우 로우
...
"낮은 케이블에서 팔꿈치를 뒤로 보내며 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Low Row
...
"From the low cable, drive the elbows back while rowing. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Low Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 낮은 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 미드 로우
...
"중간 높이 궤적으로 팔꿈치를 뒤로 보내며 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Mid Row
...
"Row on a mid-height path by driving the elbows back. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Mid Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 미드 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 로우
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Row
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 좌·우 독립 암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 로우 로우
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Low Row
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Low Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 좌·우 낮은 암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 체스트 서포티드 로우
...
"가슴은 패드에, 팔꿈치는 뒤로, 끝에서 1초."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Chest Supported Row
...
"Chest on the pad, elbows back, one-second squeeze."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Chest Supported Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"가슴 패드 높이, 시트, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — T바 로우 머신
...
"상체를 고정하고 팔꿈치로 바를 몸통으로 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — T-Bar Row
...
"Brace the torso and row the bar to the body with the elbows. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 T-Bar Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"가슴 패드, 스탠스, 플레이트, 손잡이","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 풀오버
...
"갈비뼈를 내린 채 팔꿈치를 큰 호로 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Pullover
...
"Keep the ribs down and sweep the elbows through a large arc. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Pullover 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 어시스트 풀업 / 친업
...
"보조 무게로 턱걸이 깊이를 통제하며 가슴을 바 쪽으로. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Assisted Pull-up / Chin-up
...
"Use assist load to control chin-up depth and pull the chest to the bar. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Assisted Pull-up / Chin-up 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"무릎/보조 패드, 그립, 보조 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 숄더 프레스
...
"코어를 고정한 채 손잡이를 귀 옆에서 위로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Shoulder Press
...
"Brace the core and press the handles up from beside the ears. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Shoulder Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 등판, 손잡이 시작, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 숄더 프레스
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Shoulder Press
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Shoulder Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 좌·우 독립 암, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 플레이트로드 숄더 프레스
...
"팔꿈치를 앞으로 과도히 보내지 않고 위로 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Plate-Loaded Shoulder Press
...
"Press upward without letting the elbows drift too far forward. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Plate-Loaded Shoulder Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 양쪽 플레이트, 손잡이","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 레터럴 레이즈
...
"팔꿈치를 약간 굽힌 채 측면으로 어깨 높이까지 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Lateral Raise
...
"With soft elbows, raise the arms laterally to shoulder height. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Lateral Raise 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔 패드 높이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 머신 레터럴 레이즈
...
"덤벨처럼 흔들지 말고 패드 궤적을 따라 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Machine Lateral Raise
...
"Follow the pad path and avoid swinging like dumbbells. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Machine Lateral Raise 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 암 패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 리어 델트
...
"가슴을 고정하고 후면 삼각으로 팔을 뒤로 벌리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Rear Delt
...
"Brace the chest and open the arms back with the rear delts. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Rear Delt 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴 패드, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 프론트 레이즈
...
"전면 삼각으로 팔을 앞으로 어깨 높이까지 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Front Raise
...
"Raise the arms forward to shoulder height with the front delts. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Front Raise 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/스탠스, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 업라이트 로우
...
"팔꿈치를 위로 리드하며 바를 가슴 높이로 당기기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Upright Row
...
"Lead with the elbows and pull the bar to chest height. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Upright Row 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"스탠스, 그립 폭, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 로테이터 머신
...
"팔꿈치를 옆구리에 고정한 채 천천히 외·내회전. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Rotator Machine
...
"Keep the elbow fixed at the side and rotate slowly in and out. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Rotator Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔꿈치 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 숄더 프레스 / 레터럴 복합 머신
...
"선택한 모드에 맞춰 프레스 또는 측면 거상을 분리 수행. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Shoulder Press / Lateral Combo
...
"Perform press or lateral raise separately according to the selected mode. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Shoulder Press / Lateral Combo 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 모드 전환, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 레그 프레스
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Leg Press
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Leg Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/등판 각도, 풋 플랫폼, 안전 스토퍼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 45도 레그 프레스
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — 45° Leg Press
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 45° Leg Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"등판, 풋 플랫폼 위치, 안전 스토퍼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 수평 레그 프레스
...
"골반은 시트에, 발 전체로 수평으로 밀고, 복귀는 2~3초."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Horizontal Leg Press
...
"Pelvis glued, whole-foot horizontal drive, 2–3 sec return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Horizontal Leg Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 풋 플랫폼, 안전 스토퍼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 핵 스쿼트
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Hack Squat
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Hack Squat 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"어깨 패드, 등판, 풋 플랫폼, 안전 스토퍼","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 스쿼트 프레스
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Squat Press
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Squat Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"어깨/등 패드, 풋 플랫폼, 안전 스토퍼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 벨트 스쿼트
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Belt Squat
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Belt Squat 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"벨트 높이, 풋 스탠스, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 레그 익스텐션
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Leg Extension
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Leg Extension 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 무릎 축 정렬, 발목 롤러, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 시티드 레그 컬
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Seated Leg Curl
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Seated Leg Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 무릎 축, 발목 롤러, 허벅지 패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 라잉 레그 컬
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Lying Leg Curl
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Lying Leg Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"골반 패드, 발목 롤러, 무릎 축, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 스탠딩 레그 컬
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Standing Leg Curl
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Standing Leg Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"골반/허벅지 지지, 발목 롤러, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 싱글 레그 컬
...
"발 전체로 밀고, 골반은 끝까지 안정적으로."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Single Leg Curl
...
"Whole-foot drive, pelvis stays honest."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Single Leg Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/지지, 발목 롤러, 무릎 축, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 힙 쓰러스트
...
"턱을 살짝 당긴 채 엉덩이를 천장으로 밀어 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Hip Thrust
...
"Tuck the chin slightly and drive the hips up toward the ceiling. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Hip Thrust 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"등/어깨 패드, 벨트/패드, 풋 스탠스, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 글루트 드라이브
...
"발뒤꿈치로 밀며 엉덩이만 들어 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Glute Drive
...
"Drive through the heels and lift only with the glutes. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Glute Drive 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"등판, 풋 플랫폼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 글루트 킥백
...
"골반을 고정한 채 다리를 뒤로 차며 둔근 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Glute Kickback
...
"Keep the pelvis stable and kick the leg back to squeeze the glute. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Glute Kickback 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"패드 높이, 스탠스, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 힙 어브덕션
...
"상체를 고정하고 무릎을 바깥으로 벌리며 둔근 중부 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Hip Abduction
...
"Brace the torso and open the knees outward to squeeze the glute medius. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Hip Abduction 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 무릎/허벅지 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 힙 어덕션
...
"무릎을 천천히 모으며 내전근을 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Hip Adduction
...
"Slowly bring the knees together and squeeze the adductors. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Hip Adduction 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 무릎/허벅지 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 글루트 / 힙 머신
...
"선택한 동작(어브덕션/어덕션/킥백)에 맞춰 골반을 고정. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Glute / Hip Machine
...
"Brace the pelvis for the selected move (abduction/adduction/kickback). One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Glute / Hip Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 패드, 모드/가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 스탠딩 카프
...
"발볼로만, 꼭대기 1초, 튕기지 않기."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Standing Calf
...
"Balls of the feet only, pause on top, no bounce."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Standing Calf 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"어깨 패드, 풋 플랫폼 위치, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 시티드 카프
...
"발볼로만, 꼭대기 1초, 튕기지 않기."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Seated Calf
...
"Balls of the feet only, pause on top, no bounce."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Seated Calf 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"무릎 패드, 풋 플랫폼, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 레그 프레스 카프
...
"발볼로만, 꼭대기 1초, 튕기지 않기."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Leg Press Calf
...
"Balls of the feet only, pause on top, no bounce."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Leg Press Calf 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"풋 플랫폼 위치, 무릎 잠금, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 바이셉 컬
...
"팔꿈치를 패드에 고정한 채 손잡이를 어깨 쪽으로 컬. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Biceps Curl
...
"Keep the elbows on the pad and curl the handles toward the shoulders. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Biceps Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔꿈치 패드, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 프리처 컬
...
"상완을 패드에 붙인 채 하단에서 통제하며 컬. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Preacher Curl
...
"Keep the upper arms on the pad and curl with control from the bottom. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Preacher Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 프리처 패드 높이, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 아이소래터럴 바이셉 컬
...
"좌우 같은 속도, 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Iso-Lateral Biceps Curl
...
"Match sides, one-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Iso-Lateral Biceps Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 좌·우 독립 암, 팔꿈치 패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 암 컬
...
"어깨를 고정하고 전완만 들어 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Arm Curl
...
"Keep the shoulders still and lift only with the forearms. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Arm Curl 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔꿈치 지지, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 트라이셉스 익스텐션
...
"팔꿈치를 고정한 채 전완만 펴 삼두 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Triceps Extension
...
"Keep the elbows fixed and extend only the forearms to squeeze the triceps. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Triceps Extension 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 팔꿈치 패드, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 트라이셉스 프레스
...
"팔꿈치를 몸통 가까이 두고 아래로 밀어 삼두 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Triceps Press
...
"Keep elbows close and press down to squeeze the triceps. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Triceps Press 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 딥 / 트라이셉스 머신
...
"가동범위 하단에서 어깨를 보호하며 밀어 올리기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Dip / Triceps Machine
...
"Protect the shoulders at the bottom and press back up through the range. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Dip / Triceps Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 바이셉스 / 트라이셉스 복합 머신
...
"컬과 익스텐션을 모드별로 분리해 팔꿈치만 움직이기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Biceps / Triceps Combo
...
"Separate curl and extension modes and move only at the elbow. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Biceps / Triceps Combo 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 모드 전환, 패드/손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 앱 크런치
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Ab Crunch
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Ab Crunch 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 가슴/어깨 패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 어브도미널
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Abdominal
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Abdominal 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 로터리 토르소
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Rotary Torso
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Rotary Torso 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 무릎/골반 고정, 회전 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 사이드 밴드
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Side Bend
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Side Bend 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트/힙 패드, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 백 익스텐션
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Back Extension
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Back Extension 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"골반 패드, 풋 지지, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 힙 익스텐션
...
"골반을 고정하고 다리를 뒤로 펴 둔근·햄스트링 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Hip Extension
...
"Brace the pelvis and extend the leg back to squeeze glute and hamstring. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Hip Extension 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"패드, 스탠스, 가동 범위, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 복근 / 허리 복합 머신
...
"목 말고 몸통으로, 끝에서 조이고 천천히."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Abs / Back Combo
...
"Torso curl, squeeze, slow return — not the neck."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Abs / Back Combo 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"시트, 모드 전환, 패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 케이블 크로스오버
...
"갈비뼈를 내린 채 양손을 몸 앞에서 모아 가슴 수축. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Cable Crossover
...
"Keep the ribs down and bring both hands together in front to squeeze the chest. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Cable Crossover 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"풀리 높이, 케이블 손잡이, 중량 스택","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 듀얼 어저스터블 풀리
...
"풀리 높이를 동작에 맞춘 뒤 코어를 고정하고 당기거나 밀기. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Dual Adjustable Pulley
...
"Set pulley height for the move, brace the core, then pull or press. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Dual Adjustable Pulley 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"좌·우 풀리 높이, 손잡이, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 멀티 정글짐
...
"사용 스테이션만 잡고 동선이 겹치지 않게 수행. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Multi Jungle Gym
...
"Use only your station and keep the movement path clear of others. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Multi Jungle Gym 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"스테이션 선택, 시트/패드, 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 어시스트 풀업 / 딥
...
"보조 무게로 풀업 또는 딥을 통제된 깊이로 수행. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Assisted Pull-up / Dip
...
"Use assist load to perform pull-ups or dips with controlled depth. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Assisted Pull-up / Dip 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"무릎/보조 패드, 그립, 보조 중량","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 스미스 머신
...
"바가 레일을 따라가게 두고 발 위치만 안정히 조정. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Smith Machine
...
"Let the bar follow the rails and only adjust a stable foot stance. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Smith Machine 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"바 훅, 안전 스토퍼, 바 경로","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 파워 랙
...
"세이프티를 가동범위 바로 아래에 두고 스쿼트·프레스. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Power Rack
...
"Set safeties just below your range and squat or press. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Power Rack 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"J-훅, 안전 바, 스팟터 암","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
SET
  pro_tips = jsonb_build_object(
    'ko', jsonb_build_array($k$🏋️ TGS STRENGTH — 하프 랙
...
"오픈 랙에서 바 높이를 맞춘 뒤 안정적으로 언랙. 끝에서 1초, 복귀는 통제."$k$),
    'en', jsonb_build_array($e$🏋️ TGS STRENGTH — Half Rack
...
"Set bar height on the open rack and unrack with a stable stance. One-second peak, controlled return."$e$)
  ...
  pro_tips_meta = $meta${"verificationStatus":"BRAND_MODEL_NOT_FOUND","verifiedModel":null,"manufacturer":"TGS Strength","productSeries":null,"sourceUrl":null,"verifiedStructure":"TGS Strength(TGS 스트렝스) 공개 웹·B2B 카탈로그에서 Half Rack 전용 SKU·모델코드를 확인하지 못했습니다(브랜드 공식 사이트·카탈로그 미공개)","verifiedAdjustments":"J-훅, 안전 바, 스탠스","importedAt":"2026-08-20T05:10:23.422Z"}$meta$::jsonb,
  updated_at = NOW()
FROM brands b
WHERE b.id = m.brand_id
  AND b.code = 'TGS_STRENGTH'
...
BEGIN
  ...
  IF updated_count < 80 THEN
    RAISE EXCEPTION 'TGS_STRENGTH trainer PRO tips import incomplete: % / 80', updated_count;
  END IF;
END $$;
