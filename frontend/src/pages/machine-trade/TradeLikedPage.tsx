...
export function TradeLikedPage() {
  const { t } = useTranslation('trade');
  const [params, setParams] = useSearchParams();
  const page = Math.max(1, parseInt(params.get('page') || '1', 10) || 1);

  const { data, isLoading } = useQuery({
    queryKey: QUERY_KEYS.machineTrades({ page, likedOnly: true }),
    queryFn: async () =>
      (await machineTradeApi.list({ page, limit: 20, likedOnly: true })).data.data,
  });

  return (
    <PageShell title={t('likedTrades')}>
      {isLoading ? (
