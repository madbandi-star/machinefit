...
export const workoutLogRepository = {
  async findByUserMachineDate(
    ...
    const result = await pool.query<WorkoutLogRow>(
      `SELECT wl.id, wl.recommendation_id, wl.log_date, wl.set_count, wl.set_weights_kg,
  ...
  async upsert(
    ...
    const result = await pool.query<WorkoutLogRow>(
      `INSERT INTO workout_logs (
         user_id, machine_id, recommendation_id, log_date, set_count, set_weights_kg
       )
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING id, recommendation_id, log_date, set_count, set_weights_kg,
