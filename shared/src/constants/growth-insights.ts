...
export const WEIGHT_PEER_RANGE_KG = 5;
...
export function getPeerWeightRange(weightKg: number): { weightMinKg: number; weightMaxKg: number } {
  return {
    weightMinKg: Math.max(MIN_BODY_WEIGHT_KG, weightKg - WEIGHT_PEER_RANGE_KG),
    weightMaxKg: Math.min(MAX_BODY_WEIGHT_KG, weightKg + WEIGHT_PEER_RANGE_KG),
  };
}
